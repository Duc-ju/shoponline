	...
	public int add(Book t) {
		try {
         	PreparedStatement preparedStatement = con.prepareStatement("INSERT INTO `book`"
                      	...
                      	+ " (?, ?, ?, ?, ?, ?, ?, ?);");
         	int CategoryID = new CategoryImpl().add(t.getCategory());
         	int PublisherID = new PublisherDAOImpl().add(t.getPublisher());
         	int AuthorID = new AuthorDAOImpl().add(t.getAuthor());
